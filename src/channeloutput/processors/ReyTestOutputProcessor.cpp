...
ReyTestOutputProcessor::ReyTestOutputProcessor(const Json::Value& config) {
    ...
    LogInfo(VB_CHANNELOUT, "ReyTest Remapped Channels:   %d-%d => %d-%d (%d total channels copied in %d loop(s))\n",
...
ReyTestOutputProcessor::ReyTestOutputProcessor(int src, int dst, int c, int l, int r) {
    active = true;
    sourceChannel = src;
    destChannel = dst;
    count = c;
...
ReyTestOutputProcessor::~ReyTestOutputProcessor() {
}
void ReyTestOutputProcessor::GetRequiredChannelRanges(const std::function<void(int, int)>& addRange) {
...
void ReyTestOutputProcessor::ProcessData(unsigned char* channelData) const {
    switch (reverse) {
    case 0: // No reverse
